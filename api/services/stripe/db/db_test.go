...
func TestMain(m *testing.M) {
	...
	for _, id := range ids {
		_, _ = dbc.Exec("DELETE FROM spending_unit WHERE user_external_id = $1", id)
		_, _ = dbc.Exec("DELETE FROM free_credit WHERE user_external_id = $1", id)
		_, _ = dbc.Exec("DELETE FROM invalid_subscription WHERE user_external_id = $1", id)
		_, _ = dbc.Exec("DELETE FROM user_account WHERE user_external_id = $1", id)
	}
	// Run tests
	m.Run()
}
...
func TestInsertAndGetUserAccount(t *testing.T) {
	id := "db-test-board"
	// cleanup
	defer database.GetDB().Exec("DELETE FROM free_credit WHERE user_external_id = $1", id)
	defer database.GetDB().Exec("DELETE FROM user_account WHERE user_external_id = $1", id)
	defer database.GetDB().Exec("DELETE FROM invalid_subscription WHERE user_external_id = $1", id)
	...
	err = database.GetDB().QueryRow("SELECT stripe_subscription_id FROM invalid_subscription WHERE user_external_id = $1", id).Scan(&invSub)
	if err != nil {
...
func TestFreeCreditLifecycle(t *testing.T) {
	...
	_, err = database.GetDB().Exec("UPDATE free_credit SET credit = 10 WHERE user_external_id = $1", id)
	if err != nil {
...
func TestCountUnitsBetween(t *testing.T) {
	...
	// cleanup
	defer database.GetDB().Exec("DELETE FROM spending_unit WHERE user_external_id = $1", boardID)
	defer database.GetDB().Exec("DELETE FROM user_account WHERE user_external_id = $1", boardID)
...
func TestDuplicateBoardID(t *testing.T) {
	...
	err = database.GetDB().QueryRow(
		"SELECT stripe_subscription_id, stripe_plan_id, stripe_customer_id FROM invalid_subscription WHERE user_external_id = $1", id,
